        ...
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="hidden sm:block">
              <div className="text-base font-bold leading-tight">КПК «ФИН ФОРМУЛА»</div>
            ...
            {navItems.map((item) => (
              <div key={item.href} className="relative">
                {item.submenu ? (
                  ...
                  >
                    {item.label}
                    <Icon name="ChevronDown" size={14} className={`transition-transform ${openDropdown === item.href ? 'rotate-180' : ''}`} />
                  </button>
                ) : (
                  <Link
                    to={item.href}
