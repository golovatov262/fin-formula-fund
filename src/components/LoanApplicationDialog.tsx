      ...
      <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800 rounded-xl p-4 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div>
              ...
              <div className="font-semibold">{loanProgram}</div>
            </div>
            <div>
              <div className="text-muted-foreground text-xs">Сумма</div>
              <div className="font-semibold">{loanAmount}</div>
            </div>
            <div>
              <div className="text-muted-foreground text-xs">Срок</div>
              <div className="font-semibold">{loanMonths}</div>
            </div>
          </div>
          <div className="pt-1 border-t border-orange-200 dark:border-orange-800 flex justify-between items-center">
            <span className="text-xs text-muted-foreground">Ежемесячный платёж</span>
            <span className="font-bold text-orange-600">{loanMonthlyPayment}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-1">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="la-inn">ИНН организации (ИП) <span className="text-destructive">*</span></Label>
          ...
          {/* Телефон */}
          <div className="space-y-1.5">
            <Label htmlFor="la-phone">Номер телефона <span className="text-destructive">*</span></Label>
            <Input
              id="la-phone"
              type="tel"
          ...
          {/* ФИО */}
          <div className="space-y-1.5">
            <Label htmlFor="la-name">ФИО контактного лица <span className="text-destructive">*</span></Label>
            <Input
              id="la-name"
              type="text"
          ...
          <Button type="submit" className="w-full gradient-orange-pink text-white" disabled={!isFormValid || isSubmitting}>
            {isSubmitting ? (
              <>
